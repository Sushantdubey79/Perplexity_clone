...
import { TavilyWebSearch } from "./class/tavilyWebSearch.js";
dotenv.config();
const app = express();
app.use(express.json());
try {
    const TavilyClient = new TavilyWebSearch();
    const ai = new GoogleGenAI({ apiKey: process.env.GOOGLE_API_KEY });
    app.post("/preplexity-ask", async (req, res) => {
        try {
            const question = req.body.query;
            const webSearchResults = await TavilyClient.SearchWeb(question);
